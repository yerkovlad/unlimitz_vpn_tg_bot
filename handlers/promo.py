...
from db.crud import get_promo, activate_promo, get_available_server, increment_server_users
...
async def process_promo_code(message: Message, state: FSMContext):
    ...
    async with SessionFactory() as session:
        ...
        if not promo.is_active:
            await message.answer(
                "❌ <b>This promo code is no longer active.</b>",
                reply_markup=back_home_inline()
            )
            return
        ...
        if promo.used_count >= promo.max_uses:
            await message.answer(
                "❌ <b>This promo code has reached its usage limit.</b>",
                reply_markup=back_home_inline()
            )
            return
        ...
        if not server:
            await message.answer(
                "❌ <b>No available servers for this location.</b>",
                reply_markup=back_home_inline()
            )
            return
        ...
        if not activated:
            await message.answer(
                "❌ <b>You have already used this promo code.</b>",
                reply_markup=back_home_inline()
            )
            return
        ...
        if not result:
            await message.answer(
                "❌ <b>Failed to create VPN config. Please contact support.</b>",
                reply_markup=back_home_inline()
            )
            return

        vless_link, client_uuid = result
